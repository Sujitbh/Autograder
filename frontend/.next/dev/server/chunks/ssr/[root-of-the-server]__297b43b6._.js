...
/** Build a payload compatible with CourseInterior / AssignmentGrading */ function buildPayload(data, courseId, published) {
    const langLabel = data.language === 'python' ? 'Python' : 'Java';
    return {
        id: `a-${Date.now()}`,
        name: data.name.trim() || 'Untitled Assignment',
        language: langLabel,
        dueDate: data.dueDate ? new Date(data.dueDate).toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10),
        submissions: 0,
        totalStudents: __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$studentData$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["COURSE_STUDENT_COUNTS"][courseId] ?? 42,
        gradedCount: 0,
        published,
        courseId,
        ...
        latePolicy: data.allowLateSubmissions ? `${data.latePenaltyAmount ?? 10}% deduction per day` : 'No late submissions',
...
/** Persist to the `createdAssignments` localStorage array. */ function saveToStorage(data, courseId, published) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__297b43b6._.js.map