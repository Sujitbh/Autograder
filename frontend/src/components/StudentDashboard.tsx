...
export function StudentDashboard() {
  ...
  const firstName =
    (user as any)?.firstName ?? user?.email?.split('@')[0] ?? 'Student';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {courses.map((course) => (
                ...
                >
                  ...
                  <div className="flex gap-4 mt-4 text-xs" style={{ color: 'var(--color-text-mid)' }}>
                    <span>{course.assignmentCount ?? 0} assignments</span>
                    <span>{course.pendingGrades ?? 0} pending</span>
                  </div>
                </button>
              ))}
            </div>
